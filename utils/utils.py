...
# #产生一个新的灰色底色的图片，然后再将之前的图片粘贴到这张图片上面，坐标以((w-nw)//2, (h-nh)//2)给定
...
#新函数
def letterbox_image(image, size):
    #iw, ih , liping = image.size
    iw, ih, liping = image.shape
    w, h = size
    scale = min(w/iw, h/ih)
    nw = int(iw*scale)
    nh = int(ih*scale)
    ...
    #image = np.resize(image,(nw, nh))
    #cv2.resize(image, image, (nw, nh), scale, scale, interpolation= cv2.INTER_CUBIC)
    #image是同等缩放后得到的图片
    image = cv2.resize(image, (nw, nh), fx=scale, fy=scale, interpolation=cv2.INTER_CUBIC)
    new_image = Image.new('RGB', size, (128,128,128))
    #将image转换格式，然后才可以使用后面的.paste语句
    image = ToPILImage()(image)
    #在新产生的new_image（灰条）上把经过resize和格式转换后的图像粘贴到中间位置
    new_image.paste(image, ((w-nw)//2, (h-nh)//2))
    return new_image
